...
class HaRTForSequenceClassification(HaRTBasePreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            output = (transformer_outputs.logits,) + transformer_outputs[1:]
            return ((transformer_outputs.ac_loss,) + output) if transformer_outputs.ac_loss is not None else output

        return SequenceClassifierOutputWithPast(
            loss=transformer_outputs.ac_loss,
